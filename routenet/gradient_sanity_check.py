...
def main():
    ...
    def locate_weight_file(model_dir, use_kan, target, explicit):
        if explicit:
            if not os.path.isfile(explicit):
                raise FileNotFoundError(f"指定的权重文件不存在: {explicit}")
            return explicit
        candidates = []
        expected = f"best_{target}_{'kan_model' if use_kan else 'model'}.weights.h5"
        expected_path = os.path.join(model_dir, expected)
        if os.path.isfile(expected_path):
            return expected_path
        # 兼容旧命名: 包含 lambda / 其他后缀的文件
        pattern = os.path.join(model_dir, f"*{target}*weights.h5")
        candidates.extend(glob.glob(pattern))
        # 递归浅层搜索（例如子目录 ckpt/）
        pattern_sub = os.path.join(model_dir, "*", f"*{target}*weights.h5")
        candidates.extend(glob.glob(pattern_sub))
        # 去重
        candidates = list(dict.fromkeys(candidates))
        if not candidates:
            available = [os.path.basename(f) for f in glob.glob(os.path.join(model_dir, '*'))]
            raise FileNotFoundError(
                "未找到权重文件。期望: {} 或匹配 *{}*weights.h5\n模型目录: {}\n已存在文件: {}".format(
                    expected, target, model_dir, available))
        # 选择最近修改的
        candidates.sort(key=lambda p: os.path.getmtime(p), reverse=True)
        print("⚠️ 未找到期望命名，使用候选权重文件: {}".format(os.path.basename(candidates[0])))
        return candidates[0]

    try:
        weight_file = locate_weight_file(args.model_dir, args.use_kan, args.target, args.weight_file)
        ...
        print("\n❌ 验证失败: {}".format(e))
        traceback.print_exc()
        print("\n调试建议: \n 1) 确认模型目录包含已训练的 *.weights.h5 文件\n 2) 若模型结构与权重不匹配，请核对训练时的超参数\n 3) 可手动指定 --weight_file")
