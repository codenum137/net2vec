...
def parse_fn(serialized):
    ...
    # 为 TF 2.9 的 padded_batch 准备：记录每个样本的 hop 序列长度
    # 这里将 paths 的长度作为 hop 序列总长度（与 links 和 sequences 对齐）
    features['hops_len'] = tf.shape(features['paths'])[0]

    return features, labels

def transformation_func(features_batch, labels_batch):
    """将 padded 批处理合并为单图，兼容 TF 2.9（无 ragged_batch）。"""

    # 将长度张量转为 int32 以便用于 sequence_mask
    n_links = tf.cast(features_batch['n_links'], tf.int32)
    n_paths = tf.cast(features_batch['n_paths'], tf.int32)
    hops_len = tf.cast(features_batch['hops_len'], tf.int32)

    # 获取各字段的最大长度（来自 padded_batch 的第二维）
    max_links = tf.shape(features_batch['capacities'])[1]
    max_paths = tf.shape(features_batch['traffic'])[1]
    max_hops = tf.shape(features_batch['paths'])[1]

    # 构造 mask
    link_mask = tf.sequence_mask(n_links, maxlen=max_links)
    path_mask = tf.sequence_mask(n_paths, maxlen=max_paths)
    hops_mask = tf.sequence_mask(hops_len, maxlen=max_hops)

    # 对每个字段进行 mask 后展平
    capacities_flat = tf.boolean_mask(features_batch['capacities'], link_mask)
    traffic_flat = tf.boolean_mask(features_batch['traffic'], path_mask)
    packets_feat_flat = tf.boolean_mask(features_batch['packets'], path_mask)

    links_flat = tf.boolean_mask(features_batch['links'], hops_mask)
    paths_flat = tf.boolean_mask(features_batch['paths'], hops_mask)
    sequences_flat = tf.boolean_mask(features_batch['sequences'], hops_mask)

    # 计算偏移量，修正链接与路径索引
    n_links_cumsum = tf.cumsum(tf.cast(features_batch['n_links'], tf.int64))
    n_paths_cumsum = tf.cumsum(tf.cast(features_batch['n_paths'], tf.int64))
    link_offsets = tf.concat([tf.constant([0], dtype=tf.int64), n_links_cumsum[:-1]], axis=0)
    path_offsets = tf.concat([tf.constant([0], dtype=tf.int64), n_paths_cumsum[:-1]], axis=0)

    # 为每个 hop 位置生成对应的 offset 向量
    link_offset_per_hop = tf.repeat(link_offsets, hops_len)
    path_offset_per_hop = tf.repeat(path_offsets, hops_len)

    adjusted_links_flat = links_flat + tf.cast(link_offset_per_hop, dtype=links_flat.dtype)
    adjusted_paths_flat = paths_flat + tf.cast(path_offset_per_hop, dtype=paths_flat.dtype)

    # 合并所有特征
    merged_features = {
        'traffic': traffic_flat,
        'capacities': capacities_flat,
        'packets': packets_feat_flat,
        'links': adjusted_links_flat,
        'paths': adjusted_paths_flat,
    ...
    merged_labels = {
        'delay': tf.boolean_mask(labels_batch['delay'], path_mask),
        'jitter': tf.boolean_mask(labels_batch['jitter'], path_mask),
        'drops': tf.boolean_mask(labels_batch['drops'], path_mask),
        'packets': tf.boolean_mask(labels_batch['packets'], path_mask),
    }
...
def create_dataset(filenames, batch_size, is_training=True):
    ...
    # 使用 padded_batch 兼容 TF 2.9（替代 ragged_batch）
    feature_padded_shapes = {
        'traffic': [None],
        'capacities': [None],
        'packets': [None],
        'links': [None],
    ...
    feature_padding_values = {
        'traffic': tf.constant(0.0, dtype=tf.float32),
        'capacities': tf.constant(0.0, dtype=tf.float32),
        'packets': tf.constant(0.0, dtype=tf.float32),
        'links': tf.constant(0, dtype=tf.int64),
        'paths': tf.constant(0, dtype=tf.int64),
        'sequences': tf.constant(0, dtype=tf.int64),
        'n_links': tf.constant(0, dtype=tf.int64),
        'n_paths': tf.constant(0, dtype=tf.int64),
        'hops_len': tf.constant(0, dtype=tf.int32),
    }
    label_padding_values = {
        'delay': tf.constant(0.0, dtype=tf.float32),
        'jitter': tf.constant(0.0, dtype=tf.float32),
        'drops': tf.constant(0.0, dtype=tf.float32),
        'packets': tf.constant(0.0, dtype=tf.float32),
    }

    ds = ds.padded_batch(
        batch_size,
        padded_shapes=(feature_padded_shapes, label_padded_shapes),
        padding_values=(feature_padding_values, label_padding_values),
